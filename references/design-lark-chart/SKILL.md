...
- **中间产物**：按路由落盘到 `workspace/chart-pipeline/<session>/`
- **最终产物**：
...
所有脚本默认把产物写到 `workspace/chart-pipeline/<session>/`，**不入 git**（`data/` 已在 `.gitignore`）。
...
1. 拿到样式参考画板 token，追加到 `workspace/chart-pipeline/reference_boards.json`。
